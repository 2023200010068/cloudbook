...
import { InvoiceItem } from "@/src/types/invoices";
...
export const CreateInvoicesForm = () => {
  ...
  const getAvailableCount = (productId: string, excludeId?: number) => {
    const totalCount = products.filter(
      (p) => p.product_id == productId
    ).length;

    const usedCount = invoiceItems
      .filter((item) => item.product_id == productId && item.id !== excludeId)
      ...
      invoiceItems.map((item) => {
        if (item.id == id) {
          ...
          if (field == "quantity" && item.product_id) {
            const availableCount = getAvailableCount(item.product_id, id);
    ...
    const sub_invoice = [
      {
        ...
        invoice_date: invoiceOptions.invoice_date,
      },
    ];
    ...
    <main className="bg-white p-5 mt-6 rounded-lg border shadow-md">
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <label
              className="text-[14px] block mb-1 text-black"
              htmlFor="invoice_id"
            >
              Invoice ID
            </label>
          ...
          <div>
            ...
            <DatePicker
              className="border border-gray-300 text-[14px] py-3 px-[10px] w-full bg-white text-black rounded-md transition-all duration-300 mt-2 hover:border-gray-400 focus:outline-none focus:border-blue-500 hover:bg-white focus:bg-white focus-visible:bg-white active:bg-white"
              type="date"
              id="invoice_date"
              format={dateFormat}
              value={dayjs(invoiceOptions.invoice_date)}
              onChange={(value) => handleDateChange(value, "invoice_date")}
              required
            />
          </div>
        ...
        <div className="mb-6 p-4 border rounded-lg">
          <div className="flex items-center justify-between">
            <h3 className="text-[15px] font-semibold mb-3 text-black">
                  ...
                  <tr key={item.id} className="border-t">
                    ...
                    <td className="px-4 py-2">
                      <input
                        ...
                        max={
                          item.product_id
                            ? getAvailableCount(item.product_id, item.id)
                            : undefined
                        }
                        step="1"
        ...
        <div className="flex md:flex-row flex-col md:items-start items-end md:gap-4 gap-0">
          <div className="w-full">
            <div className="mb-4">
              <label className="text-[15px] font-semibold block mb-2 text-black">
                Payment Method
              </label>
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                maxLength={250}
                className="border h-[120px] text-[14px] py-3 px-[10px] w-full bg-[#F2F4F7] hover:border-[#B9C1CC] focus:outline-none focus:border-[#B9C1CC] rounded-md transition-all duration-300 mt-2"
                id="notes"
                value={notes}
