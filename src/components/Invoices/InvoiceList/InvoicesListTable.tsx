...
import { InvoiceData, InvoiceItem, InvoicesTableProps } from "@/src/types/invoices";
...
export const InvoicesListTable: React.FC<InvoicesTableProps> = ({
  ...
  const columns: TableColumnsType<InvoiceData> = [
    ...
    },
    {
      title: "Items",
      dataIndex: "items",
      render: (items: InvoiceItem[] | string | null) => {
        let parsedItems: InvoiceItem[] | null = null;

        try {
          if (Array.isArray(items)) {
            parsedItems = items; // jsonb already parsed
        ...
        if (!Array.isArray(parsedItems)) return "N/A";
