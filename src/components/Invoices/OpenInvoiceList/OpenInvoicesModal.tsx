...
import { EditInvoiceModalProps } from "@/src/types/invoices";
...
export const OpenInvoicesModal: React.FC<EditInvoiceModalProps> = ({
  ...
  const handleDateChange = (value: dayjs.Dayjs | null, field: "invoice_date") => {
    if (value) {
      setInvoiceOptions({
        ...invoiceOptions,
  ...
  return (
    ...
    >
      ...
      <div className="grid md:grid-cols-2 grid-cols-1 gap-3">
        ...
        <div>
          <label className="text-[14px]" htmlFor="invoice_date">
            Invoice Date
          </label>
          <DatePicker
            className="border text-[14px] py-3 px-[10px] w-full bg-[#F2F4F7] hover:border-[#B9C1CC] focus:outline-none focus:border-[#B9C1CC] rounded-md transition-all duration-300 mt-2"
